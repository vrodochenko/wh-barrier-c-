...
static int compute_price(double tt, double H, double K, double r_premia, double v0, double kappa, double theta, double sigma, double rho, 
	double L, int M, int Nt) /*fill matrices with initial conditions and creates basic arrays*/
{
	/*Variables*/
	int j, n, k;
	...
	double treshold = 1e-9; /*when we assume probability to be zero and switching to a different equation*/

	int k_d, k_u;
	double sigma_local, gamma; /*wh parameters*/
	double beta_minus, beta_plus; /*wh-factors*/
	...
	for (j = 0; j < M; j++)
	{
		...
		ba_prices[j] = H*exp(ba_log_prices[j]);
	}
	dt = tt / double(Nt);
